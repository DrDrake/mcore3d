...
public:
	 ...
	 */
	void parse(sal_in_z char* source);
	...
	/*!	Get the value of attrubute with \em name for the current element.
		\return null if there is error.
	 */
	const char* attributeValue(sal_in_z const char* name) const;

	const char* attributeValueIgnoreCase(sal_in_z const char* name) const;

	float attributeValueAsFloat(size_t idx, float defaultValue = 0.0f) const;

	float attributeValueAsFloat(sal_in_z const char* name, float defaultValue = 0.0f) const;

	float attributeValueAsFloatIgnoreCase(sal_in_z const char* name, float defaultValue = 0.0f) const;

	bool attributeValueAsBool(size_t idx, bool defaultValue = false) const;

	bool attributeValueAsBool(sal_in_z const char* name, bool defaultValue = false) const;

	bool attributeValueAsBoolIgnoreCase(sal_in_z const char* name, bool defaultValue = false) const;
	//@}
	 ...
	 */
	static float stringToFloat(sal_in_z const char* str, float defaultValue = 0.0f);

	static bool stringToBool(sal_in_z const char* str, bool defaultValue = false);
