...
SemanticMap::Semantic SemanticMap::index() const		{ static const Semantic map = { "index", MeshBuilder::TYPE_UINT16, sizeof(uint16_t), 1, 0 }; return map; }
SemanticMap::Semantic SemanticMap::position() const		{ static const Semantic map = { "position", MeshBuilder::TYPE_FLOAT, sizeof(float), 3, 0 }; return map; }
...
SemanticMap::Semantic SemanticMap::uv(size_t index, size_t elementCount) const
{
	...
	Semantic map = { "", MeshBuilder::TYPE_FLOAT, sizeof(float), elementCount, index };
...
SemanticMap::Semantic SemanticMap::color(size_t index, size_t elementCount, size_t elementSize) const
{
	...
	Semantic map = { "", MeshBuilder::TYPE_UINT8, elementSize, elementCount, index };
	if(index < MCD_COUNTOF(str) && elementCount > 0 && elementCount <= 4) map.name = str[index];
	return map;
}
